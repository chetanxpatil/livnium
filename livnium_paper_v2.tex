...
\title{Three Laws of Semantic Collapse:\\NLI as Gradient Descent on an Anchor Energy Landscape}
...
% ═══════════════════════════════════════════════════════════════════════════
\begin{abstract}
We present Livnium, a classification system for Natural Language Inference
in which inference is not a single forward pass but a dynamical process:
the hidden state evolves through multiple steps of geometry-aware updates
before readout. We discover that the trained system's behavior is governed
by three empirical laws: (1) the initial state encodes the relational
difference between premise and hypothesis, (2) the semantic space is
organized by an energy landscape $V(\vech) = -\lse(\beta \cdot \cos(\vech, \anchor{k}))$
over three class anchors, and (3) the state evolves by gradient descent
on this energy. Critically, Laws~2 and~3 were not designed---they were
recovered from analyzing what a trained system was doing. We show that
replacing the trained 1.2M-parameter MLP update with the three-line
analytical gradient of $V$ matches accuracy on the full SNLI dev set
(9,842 samples), with the learned residual contributing $\leq 0.25\%$.
Joint retraining under the discovered dynamics reduces head--dynamics
disagreement from 55.1\% to 4.0\% and improves neutral recall
monotonically across five epochs ($+6.1$pp total). Trajectory analysis reveals a universal fixed-point phenomenon:
collapse dynamics converge to the same state after ${\sim}3$ steps
regardless of input, identifying the primary bottleneck. Code and
pretrained weights are publicly available.
\end{abstract}
...
The standard neural classifier computes $y = \text{softmax}(Wh + b)$:
a single affine map from representation to logits. We explore an
alternative in which the classification head is a small dynamical system
that \emph{evolves} the representation through $L$ steps before producing
a label:
\[
    h_0 \;\to\; h_1 \;\to\; \cdots \;\to\; h_L \;\to\; \text{logits}
\]
where each step applies learned residuals and geometric forces from
label-specific anchor vectors. The label is determined by which
\emph{basin} the state settles into, not by a one-shot projection.

The original Livnium system \citep{patil2026livnium} introduced this
idea with a bag-of-words encoder, achieving 76\% on SNLI. The system
used hand-designed cosine-radial forces with a 135$^{\circ}$ mismatch
from true cosine gradients---a non-conservative force field with no
scalar potential.

In this paper, we report a surprising discovery. After upgrading to a
jointly-trained BERT bi-encoder (82.06\% accuracy), we analyzed the
trained collapse dynamics and found that they are well-approximated by
gradient descent on a simple energy function over the class anchors.
The learned MLP $\delta_\theta$, which constitutes 1.2M parameters of
the collapse engine, can be replaced by a three-line analytical gradient
formula with no accuracy loss---and slight improvement.

This leads to the \textbf{Three Laws of Livnium}:
\begin{enumerate}
    \item \textbf{Law 1} (Relational State): $h_0 = v_h - v_p$
    \item \textbf{Law 2} (Energy Landscape): $V(h) = -\lse\big(\beta \cdot \cos(h, \anchor{E}),\; \beta \cdot \cos(h, \anchor{C}),\; \beta \cdot \cos(h, \anchor{N})\big)$
    \item \textbf{Law 3} (Collapse Dynamics): $h_{t+1} = h_t - \alpha \nabla V(h_t)$
\end{enumerate}

Laws 2 and 3 were not assumed---they were recovered empirically from a
trained system. This is the main result of the paper.

We further validate this finding through joint retraining experiments,
trajectory analysis via a ``tunnel test'' diagnostic, and basin stability
measurements. Trajectory analysis reveals that the current dynamics
converge to a universal fixed point after approximately three steps,
independent of input---identifying the primary bottleneck for future work.

\paragraph{Contributions.}
\begin{enumerate}
    \item Discovery that trained attractor dynamics approximate gradient
          descent on a logsumexp cosine energy (Section~\ref{sec:discovery}).
    \item Formulation of three empirical laws that fully specify the
          system's inference behavior (Section~\ref{sec:laws}).
    \item Joint retraining framework that achieves head--dynamics
          consistency (Section~\ref{sec:joint}).
    \item Tunnel test diagnostic that classifies error modes by
          trajectory analysis (Section~\ref{sec:tunnel}).
    \item Identification of a universal fixed-point phenomenon as the
