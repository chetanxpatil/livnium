...
|---|---|---|
| **1. The real core** | `packages/livnium-core/src/livnium_core/` | A reversible symbolic-geometric system. Proven. |
| **2. Honest evidence** | `tests/`, `benchmarks/nli/` | Proof the core works; honest ML benchmark. |
| **3. Experiments** | `research/archive/cortex-v2/`, `research/nli/supervised-collapse/` | Research prototypes; the big claim is **not** proven yet. |
...
## 1. `packages/livnium-core/src/livnium_core/` — the real, reusable system
...
## 3. `benchmarks/nli/` — the honest ML benchmark
...
## 4. `research/archive/cortex-v2/` — experiment: state-vector simulator
...
### `research/archive/cortex-v2/lattice.py` — reduced 3×3×3 lattice + SU(2) bridge
...
## 5. `research/nli/supervised-collapse/` — experiment: embedding / collapse trainer
...
- **`docs/collapse/COLLAPSE_ENGINE_VERDICT.md`** — the post-mortem of the collapse/cortex
...
- **`pyproject.toml`** — package config (name, version, build, ruff/black/pytest
  config).
- **`requirements.txt`** — dependencies for **reproducing `benchmarks/nli/`** only:
  ...
  `research/nli/supervised-collapse/` additionally needs `torch` + `tqdm`, and `research/archive/cortex-v2/` needs
...
- **`apps/core-visualizer/index.html`** — interactive 3D view of the cube cells, exposure
