...
## Priority 8 — Manifold Contraction Before Dynamics (v3 Extension)

### The Core Idea

The current system runs energy dynamics on the full h ∈ ℝ^768 representation.
The v3 hypothesis: **collapse the search space first, then run dynamics on the contracted state.**

Two candidate contraction operators:
- **Anchor-plane projection** — project h onto the subspace spanned by A_E, A_C, A_N (rank-3 compression)
- **Low-rank PCA/SVD** — project h onto the top-k principal directions of the anchor geometry

The key insight from external review: *"the difficulty does not disappear, it moves into the collapse operator."*
This is the honest version of the claim. Hardness is relocated, not eliminated.

### Paper-Safe Core Claim

> We test whether task-aligned manifold contraction can simplify the effective
> inference landscape by pruning nuisance dimensions before iterative
> energy-based dynamics.

This is defensible. It does not claim tensor contraction solves combinatorial hardness.
It claims contraction is worth testing as a preprocessing step.

### The 5-Condition Ablation

| Condition | Description |
|---|---|
| A | Baseline: current Livnium dynamics (no contraction) |
| B | Anchor-plane projection only (no dynamics) |
| C | Low-rank PCA/SVD projection only (no dynamics) |
| D | Anchor-plane projection → then dynamics |
| E | Low-rank projection → then dynamics |

Metrics for each: accuracy, neutral recall, steps to convergence.

Hypothesis: D > A (contraction helps dynamics) is the strongest possible result.
If D ≈ A: contraction is neutral (dynamics are already implicitly using anchor subspace).
If D < A: projection destroys information needed by Law 3.

### What "Anchor-Plane Projection" Actually Means

```python
# Project h onto subspace spanned by A_E, A_C, A_N
A = torch.stack([A_E, A_C, A_N], dim=0)  # [3, 768]
U, _, _ = torch.linalg.svd(A, full_matrices=False)  # [3, 768] → rank 3
h_proj = (h @ U.T) @ U  # project into and back out of subspace
```

**Caution (from external review):** "Neutral might need extra dimensions not perfectly spanned by E/C/N."
Softer version to test first: instead of hard projection, add a learnable mixing weight λ:

```python
h_mixed = (1 - lam) * h + lam * h_proj  # lam ∈ [0, 1]
```

Sweep λ ∈ {0, 0.1, 0.25, 0.5, 0.75, 1.0} and measure neutral recall.
This is safer than hard anchor-plane deletion.

### What NOT to claim

- ❌ "Anchor-plane projection solves the hard cases" — unproven
- ❌ "Tensor contraction guarantees dimensionality reduction benefit" — not a theorem
- ❌ "This is a Ramsey solver" — not this paper
- ✅ "We test whether restricting dynamics to an anchor-aligned low-rank subspace improves class separation" — this is the paper

### Entry Condition

Run this after: multi-seed from scratch (Priority 1b) is done.
Reason: if the laws don't replicate across seeds, v3 contraction extension is premature.
