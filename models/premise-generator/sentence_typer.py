...
Run: python3 models/premise-generator/sentence_typer.py
...
CHAR_CKPT = os.path.join(MODEL_DIR, "char_typer_pure.pt")
SENTENCE_CKPT = os.path.join(MODEL_DIR, "sentence_typer.pt")
...
def main():
    ...
    torch.save({"word_anchors": model.word_anchors.detach().cpu(),
                ...
                "config": {"dim": DIM, "n_words": n_words, "bind": args.bind}}, SENTENCE_CKPT)
    print(f"\nsaved -> {SENTENCE_CKPT}")
