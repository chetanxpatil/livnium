...
The canonical local path is **`model_nli_v1/nli_epoch20.pt`**. It is gitignored,
not bundled in a fresh clone; its expected byte size and SHA-256 are recorded in
`artifacts/checkpoints.md`. The dev-selected run achieves **68.87% SNLI test
accuracy** and **69.76% dev accuracy**, clearing the hypothesis-only artifact
baseline (61.5%).
...
python3 -m pip install -e "packages/vector-collapse[runtime]"
```

### Run a Single Prediction
To run a classification for a single sentence pair:
```bash
python3 models/collapse-nli/predict.py \
    --ckpt models/collapse-nli/model_nli_v1/nli_epoch20.pt \
...
```bash
python3 models/collapse-nli/eval_nli.py \
    --ckpt models/collapse-nli/model_nli_v1/nli_epoch20.pt \
...
```bash
python3 models/collapse-nli/ablate_nli.py \
    --ckpt models/collapse-nli/model_nli_v1/nli_epoch20.pt \
...
```bash
python3 models/collapse-nli/save_failures.py
```
