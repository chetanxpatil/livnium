"""
Vector collapse dynamics for collapse_embed (Fast Vectorized)
...
def divergence_from_alignment(align_value: torch.Tensor) -> torch.Tensor:
    """Divergence as a function of cosine alignment to the target anchor.

    The collapse update applies a force  -strength * div * normalize(h - anchor),
    i.e. it moves h by `strength * div` in the direction (anchor - h). For that to
    be a genuine point attractor ON the anchor we need div >= 0 everywhere and
    div == 0 only at perfect alignment (align == 1):

        div = 1 - align   ->  always attractive, vanishing exactly on the anchor.

    The previous law `0.38 - align` had its equilibrium at align == 0.38, i.e. a
    *shell* ~68 degrees off the anchor, and became repulsive for align > 0.38 —
    the opposite of a gravity well. See COLLAPSE_ENGINE_VERDICT.md.
    """
...
class VectorCollapseEngine(nn.Module):
    def __init__(
        ...
        # Zero-init the LAST linear so the residual block starts as the identity
        # (delta = 0). At default init the random MLP delta (||~4.3||) overpowered
        # the collapse force (||~0.04||) by ~37x and detonated every state into
        # the ||h||=10 clamp on step one. Starting from identity lets the
        # thermodynamic force actually shape the dynamics; the MLP grows only as
        # training finds it useful. See COLLAPSE_ENGINE_VERDICT.md.
        ...
        for l_idx in [0, 1, 2]:
            ...
            if spawn_new:
                maybe_spawn_vectorized(
                    ...
                    align_sub,
                    global_step,
                    self.basin_tension_threshold,
                    self.basin_align_threshold,
                )

        # 2. Dynamics loop
        # Now we have `target_centers` (B, dim) for the whole batch

        trace = {"align": [], "div": [], "tens": []}
