...
This is exactly `collapse_step` in `research/exact-gradient/pure_reply.py`, and the single-anchor special case of the NLI energy $V(h) = -\log\sum_k \exp(\beta\cos(h,A_k))$. Porting it into the noun read is the honest "grad-V" upgrade; the best that can be said for the *current* noun force is that it approximates a cubic distance well $E_{\text{euc}} \approx \tfrac{s}{6}\|h-c\|^3$ near the unit sphere.
