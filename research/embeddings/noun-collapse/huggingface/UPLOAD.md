...
the checkpoint — because it's large and lives in `research/generation/chat-brain/model/`. Copy it in
first, then upload.
...
HfApi().upload_folder(folder_path="hf_noun_upload", repo_id=repo_id,
                      repo_type="model")
```
...
- Don't commit `noun_collapse_pure.pt` into the Livnium git repo — it's large;
  keep it in `research/generation/chat-brain/model/` (gitignored) and only copy it here at upload time.
