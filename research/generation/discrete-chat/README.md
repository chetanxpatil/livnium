...
```bash
python3 research/generation/discrete-chat/prep_data.py
```

### 2. Train the Model (Pure Mode)
Train the Pure Geometric generator:
```bash
python3 research/dynamics/exact-gradient/pure_reply.py --epochs 50 --device mps
```
This will save checkpoints locally to `research/generation/discrete-chat/model/chat_reply_pure.pt`.

### 3. Talk to the Model
Run the interactive chat console:
```bash
python3 research/dynamics/exact-gradient/pure_reply.py --chat --ckpt research/generation/discrete-chat/model/chat_reply_pure.pt
```
