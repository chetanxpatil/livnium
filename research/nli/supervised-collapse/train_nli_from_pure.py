"""
train_nli_from_pure.py — use the PURE char geometry (char_typer_pure.pt) in SNLI.

The pure char bot has no word vectors and no MLP — just 26 orthogonal letter
wells. To hand a fixed vector per word to the NLI pipeline, we pool the word's
letter anchors:

    word_vec = mean over letters of  normalize(letter_anchor[c])

Because the wells are ~orthogonal, this is a clean letter-fingerprint of the
word (a bag-of-characters embedded in the learned space). It is FROZEN — pure
geometry, nothing learned here. Meaning is learned on top, supervised:

    frozen char-bag word vectors --> MeaningHead (trainable) --> pool sentence
        --> pair = u - v --> VectorCollapseEngine (trainable) --> cosine E/N/C

Honest expectation: characters carry spelling, not sentence meaning, so this
measures what the char geometry alone supports for NLI. Your own benchmarks put
char-level Livnium at ~43% (vs word-level 60%, supervised 68.9%). This is the
collapse-engine version of that experiment — let the number speak.

Usage:
  python3 train_nli_from_pure.py --nli-path /path/to/snli_1.0_train.jsonl
Needs torch + char_typer_pure.pt (run char_collapse_pure.py first).
"""

import argparse
import json
import random

import torch
import torch.nn as nn
import torch.nn.functional as F

from vector_collapse import VectorCollapseEngine

NLI_CKPT = "model_nli_v1/nli_epoch20.pt"
PURE_CKPT = "char_typer_pure.pt"
NLI_LABEL_TO_IDX = {"entailment": 0, "neutral": 1, "contradiction": 2}
LETTERS = "abcdefghijklmnopqrstuvwxyz"
STOI = {c: i + 1 for i, c in enumerate(LETTERS)}   # matches pure model (0=pad)
ALPHA = set(LETTERS)


def load_vocab():
    try:
        data = torch.load(NLI_CKPT, map_location="cpu", weights_only=False)
    except TypeError:
        data = torch.load(NLI_CKPT, map_location="cpu")
    v = data["vocab"]
    return v["idx2word"], {w: i for i, w in enumerate(v["idx2word"])}, v["pad_idx"], v["unk_idx"]


def read_nli_jsonl(path, max_lines=0):
    out = []
    with open(path, encoding="utf-8") as f:
        for line in f:
            if max_lines and len(out) >= max_lines:
                break
            line = line.strip()
            if not line:
                continue
            d = json.loads(line)
            lab = d.get("gold_label", "-")
            if lab not in NLI_LABEL_TO_IDX:
                continue
            s1 = (d.get("sentence1") or "").lower()
            s2 = (d.get("sentence2") or "").lower()
            if s1 and s2:
                out.append((s1, s2, NLI_LABEL_TO_IDX[lab]))
    return out


def encode_line(line, w2i, unk):
    return [w2i.get(t, unk) for t in line.strip().split() if t] or [unk]


def collate(batch, pad):
    pms, hys, ys = zip(*batch)
    mp = max(len(x) for x in pms); mh = max(len(x) for x in hys)
    f = lambda seqs, m: torch.tensor([s + [pad] * (m - len(s)) for s in seqs], dtype=torch.long)
    return f(pms, mp), f(hys, mh), torch.tensor(ys, dtype=torch.long)


def build_char_bag_table(idx2word, pad_idx):
    """Frozen word vectors = mean of the pure model's letter anchors."""
    ck = torch.load(PURE_CKPT, map_location="cpu")
    anchors = F.normalize(ck["letter_anchors"], dim=-1)   # (28, dim)
    dim = anchors.size(1)
    table = torch.zeros(len(idx2word), dim)
    for i, w in enumerate(idx2word):
        if i == pad_idx:
            continue
        ids = [STOI[c] for c in w.lower() if c in ALPHA]
        if ids:
            table[i] = anchors[torch.tensor(ids)].mean(0)
    return table, dim


class MeaningNLI(nn.Module):
    def __init__(self, table, pad_idx, dim):
        super().__init__()
        self.embed = nn.Embedding.from_pretrained(table, freeze=True, padding_idx=pad_idx)
        self.pad_idx = pad_idx
        self.meaning = nn.Sequential(nn.Linear(dim, dim), nn.Tanh(), nn.Linear(dim, dim))
        self.engine = VectorCollapseEngine(dim=dim, num_layers=4)

    def pool(self, ids):
        e = self.meaning(self.embed(ids))
        m = (ids != self.pad_idx).float().unsqueeze(-1)
        return (e * m).sum(1) / m.sum(1).clamp(min=1.0)

    def anchors(self):
        return torch.stack([
            F.normalize(self.engine.anchor_entail, dim=0),
            F.normalize(self.engine.anchor_neutral, dim=0),
            F.normalize(self.engine.anchor_contra, dim=0),
        ])

    def forward(self, prem, hyp, temp=0.1):
        pair, _ = self.engine(self.pool(prem) - self.pool(hyp))
        return (F.normalize(pair, dim=-1) @ self.anchors().t()) / temp


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--nli-path", default="/Users/chetanpatil/Desktop/test/data-bank/snli_1.0_train.jsonl")
    ap.add_argument("--epochs", type=int, default=3)
    ap.add_argument("--batch-size", type=int, default=512)
    ap.add_argument("--lr", type=float, default=1e-3)
    ap.add_argument("--max-lines", type=int, default=0)
    ap.add_argument("--dev-frac", type=float, default=0.02)
    args = ap.parse_args()

    device = torch.device("mps" if torch.backends.mps.is_available()
                          else "cuda" if torch.cuda.is_available() else "cpu")
    torch.manual_seed(0)

    idx2word, w2i, pad_idx, unk_idx = load_vocab()
    table, dim = build_char_bag_table(idx2word, pad_idx)
    nz = int((table.abs().sum(1) > 0).sum())
    print(f"vocab {len(idx2word)}  dim {dim}  device {device}")
    print(f"char-bag word vectors (FROZEN, pure geometry): {nz}/{len(idx2word)} words")
    print("trainable: meaning head + collapse engine only\n")

    model = MeaningNLI(table, pad_idx, dim).to(device)
    data = read_nli_jsonl(args.nli_path, args.max_lines)
    data = [(encode_line(s1, w2i, unk_idx), encode_line(s2, w2i, unk_idx), y) for s1, s2, y in data]
    random.seed(0); random.shuffle(data)
    n_dev = int(len(data) * args.dev_frac)
    dev, train = data[:n_dev], data[n_dev:]
    print(f"train {len(train)}   dev {len(dev)}")

    opt = torch.optim.Adam((p for p in model.parameters() if p.requires_grad), lr=args.lr)

    def acc(d):
        model.eval(); ok = tot = 0
        with torch.no_grad():
            for i in range(0, len(d), args.batch_size):
                p, h, y = collate(d[i:i + args.batch_size], pad_idx)
                ok += (model(p.to(device), h.to(device)).argmax(-1).cpu() == y).sum().item()
                tot += len(y)
        return ok / max(1, tot)

    for ep in range(1, args.epochs + 1):
        model.train(); random.shuffle(train)
        for i in range(0, len(train), args.batch_size):
            p, h, y = collate(train[i:i + args.batch_size], pad_idx)
            logits = model(p.to(device), h.to(device))
            a = model.anchors()
            sep = sum(torch.relu((a[x] * a[z]).sum()) for x, z in [(0, 1), (1, 2), (0, 2)])
            loss = F.cross_entropy(logits, y.to(device)) + sep
            opt.zero_grad(); loss.backward(); opt.step()
            if (i // args.batch_size) % 150 == 0:
                print(f"  ep{ep} step {i//args.batch_size:4d}  ce {loss.item():.4f}")
        print(f"epoch {ep}: train {acc(train[:20000])*100:.2f}%   dev {acc(dev)*100:.2f}%")


if __name__ == "__main__":
    main()
