"""
Collapse-Based Embedding Trainer (Livnium v0.1) [M5 Optimized]
...
class CollapseEmbeddingModel(nn.Module):
...
def nli_supervised_loss(
    ...
    sep = (
        torch.relu((cE * cN).sum() - sep_margin)
...
def build_vocab_and_sequences(
    path: str, max_lines: int, max_size: int
...
def sample_negative(
    batch_size: int, vocab_size: int, pad_idx: int, device: torch.device
) -> torch.Tensor:
...
def verify_engine_trained(collapse_engine, init_anchors, sep_threshold: float, epoch: int):
    ...
    print(
        f"[verify] epoch {epoch}: max|Δanchor| = "
    ...
    ccn = float((c * n).sum())
    print(
        f"[verify] epoch {epoch}: cos(E,C)={cec:+.3f} cos(E,N)={cen:+.3f} cos(C,N)={ccn:+.3f} "
...
def train(
    ...
    model = CollapseEmbeddingModel(vocab_size=len(vocab), dim=dim, pad_idx=vocab.pad_idx).to(device)
    ...
    basin_field = (
        BasinField(max_basins_per_label=basin_max_per_label).to(device)
    ...
    if collapse_layers > 0:
        ...
        print(
            f"[init] Collapse Engine Active (Layers: {collapse_layers}, Dynamic: {use_dynamic_basins})"
        )
    ...
    print(
        f"[init] optimizer tracking {sum(p.numel() for p in trainable):,} params "
    ...
    if verify_engine and collapse_engine is not None:
        ...
        init_anchors = {
            n: sd0[n].detach().clone() for n in ("anchor_entail", "anchor_contra", "anchor_neutral")
        }
    ...
    for epoch in range(1, epochs + 1):
        ...
        ckpt_path = os.path.join(output_dir, f"collapse_embed_epoch{epoch}.pt")
        torch.save(
            {
    ...
    final_path = os.path.join(output_dir, "collapse_embeddings_final.pt")
    torch.save(
        {
...
def train_nli(
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            f"[nli] epoch {epoch}: mean_loss={tot/max(global_step,1):.4f} "
...
def main():
    parser = argparse.ArgumentParser(description="Collapse-Based Embedding Trainer [M5 Optimized]")
    parser.add_argument(
        "--train-path",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--disable-dynamic-basins", action="store_true", help="Disable dynamic spawning."
    )
    ...
    lambda_sep = d.pop("lambda_sep")
    sep_margin = d.pop("sep_margin")
    temp = d.pop("temp")
    ...
    if task == "nli":
        ...
        train_nli(
            ...
            lr=d["lr"],
            device=d["device"],
            collapse_layers=d["collapse_layers"],
            strength_entail=d["strength_entail"],
            strength_contra=d["strength_contra"],
            strength_neutral=d["strength_neutral"],
            lambda_sep=lambda_sep,
            sep_margin=sep_margin,
            temp=temp,
            verify_engine=d["verify_engine"],
            sep_threshold=d["sep_threshold"],
        )
    else:
