...
% ═══════════════════════════════════════════════════════════════════════════
\begin{abstract}
We show that a trained neural update in an iterative NLI classifier
can be removed and replaced with a closed-form energy gradient---with
no retraining and no loss in performance (82.21\% vs 82.05\% on SNLI
dev, $n=9{,}842$).

We train an iterative classification head in which a hidden state
evolves through multiple update steps before label readout, implemented
as a learned MLP (1.2M parameters) trained end-to-end on SNLI.
We find that this MLP is unnecessary. Replacing it with the analytical
gradient of an energy function over anchor cosine similarities,
$V(\vech) = -\lse(\beta \cdot \cos(\vech, \anchor{k}))$,
produces identical performance without retraining. The trained
...
We verify this by deleting the trained MLP entirely and replacing it
with the analytical gradient of an energy function---no retraining,
no accuracy loss.
...
The key result is that removing the learned MLP does not degrade
performance. Table~\ref{tab:collapse} shows the analytical gradient
...
\subsection{Geometric Robustness Under MLP Divergence}

To test whether the grad-V result depends on the MLP being well-trained,
we evaluated a second independent checkpoint where the MLP had begun to
diverge from the anchor geometry. During fine-tuning from the primary
checkpoint with a different random seed, the MLP weights drifted while
the anchor structure remained intact ($\cos(A_E, A_C) = -0.188$,
$\cos(A_C, A_N) = +0.118$). At this checkpoint the full system---MLP
plus anchor forces---achieved only 70.66\% accuracy, 11.4pp below
the baseline. The MLP was actively degrading classification.
...
Mode & Accuracy & E-recall & N-recall & C-recall \\
\midrule
\multicolumn{5}{c}{\textit{Diverged-MLP checkpoint ($n=9{,}842$)}} \\
\midrule
Full ($\delta_\theta$ + forces) & 70.66\% & 60.35\% & 58.36\% & 93.26\% \\
No-delta (forces only) & 81.95\% & 86.30\% & 72.64\% & 86.73\% \\
\textbf{Grad-V ($\beta=2$, $\alpha=0.05$)} & \textbf{82.25\%} & 88.16\% & \textbf{75.67\%} & 82.73\% \\
\bottomrule
\end{tabular}
\caption{Grad-V evaluated on a checkpoint where the MLP has diverged.
The full system (MLP + forces) collapses to 70.66\%, while the
analytical gradient of $V(h)$---which ignores the MLP entirely---recovers
82.25\%. The anchor geometry outlasts the neural weights.}
\label{tab:robustness}
\end{table}

The analytical gradient recovered 82.25\%, a gap of $+11.59$pp over the
full system (Table~\ref{tab:robustness}). The anchor geometry, learned
during the original training run, persisted unchanged; only the MLP
had drifted. Because $\nabla V$ derives solely from the geometry and
ignores the neural weights entirely, it cannot be degraded by MLP
divergence. The MLP is not just redundant---it is a liability. The
geometry is the classifier.

\subsection{Stability Across \texorpdfstring{$\beta$}{beta}}

We swept $\beta \in \{0.5, 1, 2, 5, 10, 20, 50\}$ at $\alpha = 0.1$.
...
Correctly classified samples show mean energy descent $\Delta V = +0.046$
(ascending---moving toward lower-energy basins), while error samples
show $\Delta V = -0.002$ (flat---stuck at the fixed point).
The dynamics are contracting for correct predictions and stagnant
for errors.
...
\subsection{Why Does the MLP Learn \texorpdfstring{$\nabla V$}{grad V}?}
...
\subsection{What Is the Energy \texorpdfstring{$V(h)$}{V(h)}?}
