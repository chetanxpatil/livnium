...
\title{Iterative Attractor Dynamics for Classification:\\Energy-Guided Basin Settling as an Alternative to Linear Readout}
...
until it settles near one basin, and the label is read from which basin
captured it. We define a Lyapunov function $V(\vech) = (\barrier -
\cos(\vech, \anchor{y}))^2$ and prove that, in the absence of the learned
residual, the dynamics are locally contracting. The approach is evaluated
on the Stanford Natural Language Inference (SNLI) benchmark, achieving
76\% dev accuracy with a bag-of-words encoder, 428$\times$ faster than
BERT-base at inference on CPU. We report the geometric properties of the
...
\emph{does iterative basin settling outperform or complement a linear
readout, given the same input representation?}
...
all three anchors act simultaneously with their respective strengths,
and the label is determined by which basin the state settles nearest.
...
After $L$ collapse steps, the final state $h_L$ is passed to a
two-layer classification head along with auxiliary geometric features:
cosine alignment between $\bm{v}_p$ and $\bm{v}_q$, opposition signal
$\cos(-\bm{v}_p, \bm{v}_q)$, Euclidean distance $\|\bm{v}_q - \bm{v}_p\|$,
norm features, and a learned neutral alignment signal. The head outputs
logits for the three SNLI classes.
...
\begin{theorem}[Local contraction without learned residual]
...
\[
    \nabla_h \cos(h, A) \cdot \nhat(h, A)
    = -\frac{\beta \sin^2\theta}{\alpha \|h - A\|}
\]
where $\alpha = \|h\|$, $\beta = \|A\|$, and $\theta$ is the angle
...
\bottomrule
\end{tabular}
\caption{Head-only inference speed on CPU. This excludes encoding time
and is not directly comparable to end-to-end BERT timings.}
\label{tab:speed}
\end{table}
