...
def phase_anli():
    ...
    res = {}
    for i in (1, 2, 3):
        te = tests[i]; Xte = pair(list(te.premise), list(te.hypothesis))
...
# ============================================================ combine
def phase_combine():
    s = json.load(open(HERE / "_snli_res.json")) if (HERE/"_snli_res.json").exists() else None
    # results from the glove run were not persisted; read final numbers from RUNG2_RESULTS instead
    ls = json.load(open(HERE / "_liv_snli.json")); la = json.load(open(HERE / "_liv_anli.json"))
    ...
    out = ["\n\n---\n\n# Livnium's turn — the geometric encoder in the same fair harness\n",
        ...
        "## SNLI\n",
        "| model | test acc % |", "|---|---|",
        "| majority | 34.3 |", "| full BoW (prem+hyp) | 59.4 |",
        "| hypothesis-only BoW | 61.5 |", "| GloVe-100 learned | 60.7 |",
    ...
    out += ["```\n", "## ANLI (bar = hypothesis-only ~37-41%)\n",
        ...
        "|---|---|---|---|---|---|"]
    glove_anli = {1: 34.2, 2: 36.7, 3: 34.4}; hyp = {1: 39.6, 2: 36.9, 3: 36.7}; bestbow = {1: 41.3, 2: 37.8, 3: 36.8}
    for i in (1, 2, 3):
        out.append(f"| R{i} | {hyp[i]:.1f} | {bestbow[i]:.1f} | {glove_anli[i]:.1f} | "
                   f"**{la[i]['livnium']:.1f}** | {la[i]['shuf']:.1f} |")
    snli_liv = ls["livnium"]
    out += ["", "## Verdict for Livnium (measured, same rules)\n",
        ...
        + ("Clears the bar on all rounds." if all(la[i]['livnium'] > hyp[i] for i in (1,2,3))
    ...
    for i in (1,2,3):
        print(f"ANLI R{i} livnium={la[i]['livnium']:.1f}  (hypOnly {hyp[i]} / GloVe {glove_anli[i]})")
