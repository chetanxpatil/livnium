...
import re
_w = re.compile(r"[a-z']+")
def words(s): return _w.findall(s.lower())
...
def idx_to_coord(i):
    x = i % N - HALF; y = (i // N) % N - HALF; z = (i // (N * N)) % N - HALF
    return (x, y, z)
...
def occupancy(sents):
    ...
    for s in sents:
        cnt = {}
        for w in words(s):
            c = cell_of(w); cnt[c] = cnt.get(c, 0) + 1
...
def geom_dense(sents):
    ...
    for r, s in enumerate(sents):
        ws = words(s)
        if not ws: continue
        ex = np.zeros(4); mc = np.zeros(3)
        for w in ws:
            co = idx_to_coord(cell_of(w))
            ex[exposure(co, N)] += 1; mc += co
        out[r, :4] = ex / len(ws); out[r, 4:] = mc / len(ws)
    return out

def Xocc(P, H): return hstack([occupancy(P), occupancy(H)]).tocsr()
def acc(clf, X, y):
...
def phase_snli():
    ...
    trP, trH, trY = R.load_snli("train"); teP, teH, teY = R.load_snli("test")
    ...
    g = acc(clf, Xte, teY); cm = confusion_matrix(teY, clf.predict(Xte)).tolist()
    clfS = LogisticRegression(max_iter=200, solver="liblinear").fit(Xtr, np.random.RandomState(1).permutation(trY))
    shuf = acc(clfS, Xte, teY)
    # geometry-only
    Gtr = np.hstack([geom_dense(trP), geom_dense(trH)]); Gte = np.hstack([geom_dense(teP), geom_dense(teH)])
...
def phase_anli():
    ...
    clfS = LogisticRegression(max_iter=200, solver="liblinear").fit(Xtr, np.random.RandomState(1).permutation(tr.label.values))
    res = {}
    for i in (1, 2, 3):
        te = tests[i]; Xte = Xocc(list(te.premise), list(te.hypothesis))
...
# ============================================================ combine
def phase_combine():
    ws = json.load(open(HERE / "_wliv_snli.json")); wa = json.load(open(HERE / "_wliv_anli.json"))
    wa = {int(k): v for k, v in wa.items()}
    md = (HERE / "RUNG2_RESULTS.md").read_text().rstrip()
    LAB = ["entail", "neutral", "contra"]
    hyp = {1: 39.6, 2: 36.9, 3: 36.7}; bestbow = {1: 41.3, 2: 37.8, 3: 36.8}
    out = ["\n\n---\n\n# Word-level Livnium — encode the word, not the letter\n",
        ...
        "| full BoW (prem+hyp) | 59.4 |", "| GloVe-100 learned | 60.7 |",
    ...
    for i in (1, 2, 3):
        out.append(f"| R{i} | {hyp[i]:.1f} | {bestbow[i]:.1f} | **{wa[i]['occ']:.1f}** | {wa[i]['shuf']:.1f} |")
    occ = ws["occ"]
    out += ["", "## Verdict — did moving to the word unit change the number?\n",
        f"- **SNLI:** char-level Livnium 43.2 -> **word-level {occ:.1f}**. "
        + (f"Climbs +{occ-43.2:.1f} pts and "
           + ("reaches bag-of-words territory" if occ >= 57 else "moves toward bag-of-words") + ". " if occ > 43.2
           else "did not climb. ")
        ...
        + ("near chance" if ws['geom'] < 40 else "weak") + ". The accuracy comes from WORD IDENTITY occupying cells, not the geometry.\n",
    ...
    print(f"SNLI occ={occ:.1f} geom-only={ws['geom']:.1f} shuf={ws['shuf']:.1f}  (char-liv 43.2 / BoW 59.4 / GloVe 60.7 / hypOnly 61.5)")
    for i in (1,2,3): print(f"ANLI R{i} occ={wa[i]['occ']:.1f}  (hypOnly {hyp[i]} / bestBoW {bestbow[i]})")
