...
def load_snli(name):
    ...
    for line in open(SNLI_DIR/f"snli_1.0_{name}.jsonl"):
        ...
        if g in LABELS:
            P.append(d["sentence1"]); H.append(d["sentence2"]); Y.append(LABELS[g])
...
def load_anli():
    ...
    tests={i:(lambda d:d[d.label.isin([0,1,2])])(pd.read_parquet(ANLI_DIR/f"anli_test_r{i}.parquet")) for i in (1,2,3)}
    return tr,tests

# ---- fast vectorized encoder over trimmed embeddings ----
class Enc:
    def __init__(self):
        z=np.load(EMB_NPZ); self.M=z["M"]; self.dim=self.M.shape[1]
...
def phase_trim():
    ...
    for name in ("train","dev","test"):
        ...
        for s in P+H: vocab.update(toks(s))
    tr,tests=load_anli()
    import itertools
    ...
    for t in vocab:
        if t in kv:
            kept[t]=len(vecs); vecs.append(kv[t])
...
def phase_snli():
    ...
    enc=Enc(); log("snli: loading data")
    trP,trH,trY=load_snli("train"); teP,teH,teY=load_snli("test")
    idx=np.random.RandomState(SEED).permutation(len(trY))[:N_TRAIN_SNLI]
    trP=[trP[i] for i in idx]; trH=[trH[i] for i in idx]; trY=trY[idx]
    log(f"snli train={len(trY)} test={len(teY)}")
    maj=np.bincount(trY).argmax(); maj_acc=(teY==maj).mean()*100
    ...
    Xtr=enc.pair(trP,trH); Xte=enc.pair(teP,teH)
    clfG=LogisticRegression(max_iter=250,n_jobs=1,tol=1e-3).fit(Xtr,trY)
    g=acc(clfG,Xte,teY); cm=confusion_matrix(teY,clfG.predict(Xte)).tolist()
    ...
    json.dump(dict(maj=maj_acc,hyp=hyp,bow=bow,glove=g,shuf=shuf,cm=cm,
                   ntrain=int(len(trY)),ntest=int(len(teY))),
...
def phase_anli():
    ...
    vH=CountVectorizer(max_features=30000)
    clfH=LogisticRegression(max_iter=200,solver="liblinear").fit(vH.fit_transform(tr.hypothesis),tr.label)
    def fitbow(cls,**kw):
        a,b=cls(**kw),cls(**kw)
    ...
    for i in (1,2,3):
        te=tests[i]
        def ba(m): a,b,c=m; return acc(c,hstack([a.transform(te.premise),b.transform(te.hypothesis)]).tocsr(),te.label)
        Xte=enc.pair(list(te.premise),list(te.hypothesis))
        res[i]=dict(maj=(te.label==maj).mean()*100,hyp=acc(clfH,vH.transform(te.hypothesis),te.label),
                    cnt=ba(cnt),hsh=ba(hsh),tfi=ba(tfi),glove=acc(clfG,Xte,te.label),
                    shuf=acc(clfS,Xte,te.label))
        log(f"anli R{i} hyp={res[i]['hyp']:.1f} cnt={res[i]['cnt']:.1f} glove={res[i]['glove']:.1f}")
    json.dump(dict(ntrain=int(len(tr)),rounds=res), open(HERE/"_anli_res.json","w"))

def phase_combine():
    s=json.load(open(HERE/"_snli_res.json")); A=json.load(open(HERE/"_anli_res.json")); a=A["rounds"]
    a={int(k):v for k,v in a.items()}
    L=["# Rung 2 results — learned representation vs boring baselines\n",
    ...
    L+=["```\n","## ANLI (the hard, artifact-free task — honest run)\n",
        ...
        "|---|---|---|---|---|---|---|---|"]
    for i in (1,2,3):
        r=a[i]; L.append(f"| R{i} | {r['maj']:.1f} | {r['hyp']:.1f} | {r['cnt']:.1f} | {r['hsh']:.1f} | {r['tfi']:.1f} | **{r['glove']:.1f}** | {r['shuf']:.1f} |")
    L.append("")
    ...
    L+=["## Verdict (measured)\n",
        ...
        +("**Learned beats word-counting — Rung 2 SNLI gate PASSED.**" if snli_win else "Learned did NOT clear BoW.")+"\n",
        f"- **ANLI:** GloVe minus hyp-only bar: "+", ".join(f"R{i} {a[i]['glove']-a[i]['hyp']:+.1f}" for i in (1,2,3))+". "
        +("Clears the bar on all rounds." if anli_beats else "Does NOT clear the hyp-only bar on all rounds — honest near-chance, as expected on ANLI.")+"\n",
    ...
    print("\n==== SUMMARY ====")
    print(f"SNLI learned={s['glove']:.1f} BoW={s['bow']:.1f} hyp={s['hyp']:.1f} gate={'PASS' if snli_win else 'FAIL'}")
    for i in (1,2,3): print(f"ANLI R{i} learned={a[i]['glove']:.1f} hyp={a[i]['hyp']:.1f} cnt={a[i]['cnt']:.1f}")

if __name__=="__main__":
    {"trim":phase_trim,"snli":phase_snli,"anli":phase_anli,"combine":phase_combine}[sys.argv[1]]()
