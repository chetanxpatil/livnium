...
```bash
cd benchmarks/embeddings/matched-corpus
...
- `work/benchmarks/nli/*.pairs.csv` — raw per-pair predictions (the audit trail)
- `work/benchmarks/nli/aggregate.{json,csv}` — raw + mean ± std per family per dataset
