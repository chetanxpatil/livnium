...
class QuantumSNLIEncoder(nn.Module):
    ...
    alpha_first_token: symmetry-breaking injection.
        The bag-of-words mean collapses "A bites B" and "B bites A" to the
        same coordinate — perfect symmetry, no direction for the field to
        amplify. Injecting a scaled first-token difference breaks that
        symmetry at zero parameter cost:

            h0 = (v_h - v_p) + alpha * (embed(hyp[0]) - embed(prem[0]))

        The first token in an English sentence is almost always the subject.
        Even a small alpha gives the collapse engine a directional crack to
        work with. The field amplifies it from there.

        alpha=0.0  →  pure bag-of-words baseline (symmetry intact)
        alpha=0.1  →  weak signal, minimal interference with mean
    ...
    def build_initial_state(
        ...
        if self.alpha_first_token != 0.0:
            # First-token injection: embed(hyp[0]) - embed(prem[0])
            # Breaks bag-of-words symmetry using subject identity.
            te = self.text_encoder
            device = te.embed.weight.device
            p0 = prem_ids[:, 0].to(device)   # [B] — first token of premise
            h0_tok = hyp_ids[:, 0].to(device)  # [B] — first token of hypothesis
            v_p_first = te.embed(p0)           # [B, dim]
            v_h_first = te.embed(h0_tok)       # [B, dim]
