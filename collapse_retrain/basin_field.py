"""
Dynamic Basin Field for collapse_embed (Vectorized)
...
def route_to_basin_vectorized(
    ...
    # Metrics — keep consistent with vector_collapse.divergence_from_alignment
    # (attractive law: div = 1 - align, zero only on the anchor).
    best_align = best_sims
    divergence = 1.0 - best_align
    tens = divergence.abs()
        ...
        for i in range(len(active_idxs)):
            ...
            for j in range(i + 1, len(active_idxs)):
                ...
                if sims[i, j] > merge_cos_threshold:
                    ...
                    new_center = (
                        w_i * field.centers[l_idx, idx_i] + w_j * field.centers[l_idx, idx_j]
                    )
