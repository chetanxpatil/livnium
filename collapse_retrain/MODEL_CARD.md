...
experimental `collapse_retrain/` line. Given two sentences — a **premise** and a
...
(see `results/RESULTS.md`). So this is a genuine step up — for a model with **no
...
Produced by `collapse_retrain/train_collapse_embeddings.py --task nli`. Reproduce
end-to-end with the command above. See `COLLAPSE_ENGINE_VERDICT.md` for the
engine's design history and `docs/COMPONENTS.md` for how this fits the wider repo.
