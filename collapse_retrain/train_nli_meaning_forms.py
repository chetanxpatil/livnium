"""
train_nli_meaning_forms.py — meaning FORMS AROUND the char structure (supervised).

The pure char geometry gives every word a fixed position by its letters. We keep
that as a frozen scaffold and let meaning grow around each word as a trainable
residual the SNLI labels shape:

    word_vec = char_anchor(word)        # FROZEN pure-geometry letter position
...
At step 0 every word sits exactly on its char scaffold (residual 0). Training
grows the residual so synonyms drift together and E/N/C separates — meaning
literally forms around the char structure. Words unseen in training keep
residual 0 and gracefully fall back to their char position.

Trainable: meaning residual + collapse engine + anchors. Char scaffold frozen.

Usage: python3 train_nli_meaning_forms.py --nli-path /path/to/snli_1.0_train.jsonl
Needs torch + char_typer_pure.pt.
"""
...
STOI = {c: i + 1 for i, c in enumerate(LETTERS)}
ALPHA = set(LETTERS)
...
def build_char_scaffold(idx2word, pad_idx):
    ...
    anchors = F.normalize(ck["letter_anchors"], dim=-1)
    dim = anchors.size(1)
...
def main():
    ...
    print("char scaffold FROZEN; meaning residual (init 0) + engine TRAINABLE\n")
    ...
    torch.save({"model": model.state_dict(), "dim": dim}, "nli_meaning_forms.pt")
    print("\nsaved -> nli_meaning_forms.pt")
